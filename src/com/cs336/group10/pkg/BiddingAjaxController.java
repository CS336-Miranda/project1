...
public class BiddingAjaxController extends HttpServlet {
	...
	private void _getAuction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			ApplicationDB db = new ApplicationDB();	
		 	...
		 	String sqlQuery = "SELECT a.auctionId, a.highestBid, b.email, a.minPrice, a.closeTime, a.startTime, a.title, a.description, a.bidIncrement, a.initialPrice, a.owner, a.highestBidder, bs.itemId, e.name, e.company, e.year, e.color, l.touch, c.provider, t.size " + 
		 			//"CASE WHEN a.highestBid + b.bidIncrement <= b.upperLimit THEN a.highestBid + b.bidIncrement ELSE b.upperLimit " + 
		 			//"END highestBid, b.email FROM _cs336_buyme.auctions a " + 
		 			...
		 			"LEFT OUTER JOIN _cs336_buyme.bids b on (a.auctionId = b.auctionId) " +
		 			"WHERE a.auctionId = ?;";
	...
	private void _bidAddNew(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			int result2 = _updateHighBidOnAuction(request, response, con);
			
			ResultSet rs = _getHighBid(request, response, con);
			
			/*while (rs.next() && ) {
				Float upperLimit = Float.parseFloat(rs.getString("upperLimit"));
				Float bidIncrement = Float.parseFloat(rs.getString("bidIncrement"));
